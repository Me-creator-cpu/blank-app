   ...
   if st.session_state['data_srv'] is not None:
      ...
      with st.spinner("Wait for it...", show_time=True):
         ...
         with row4[0]:
            ...
            avg_df
         with row4[1]:
             avg_df = df_gr.set_index('Type').groupby('Type').apply(lambda x: x['Level'].sum() / x['Level'].count(), include_groups=False).to_frame('Type','Level')
...
def build_tile_v2(name="Caption Tile",image_url="",level=1,stars=0,skill="",type=""):
    strHtml='<span>'
    strHtml=strHtml+'<table>'
    strHtml=strHtml+'<tr><td colspan=2><img src="'+image_url+'"></td></tr>'
